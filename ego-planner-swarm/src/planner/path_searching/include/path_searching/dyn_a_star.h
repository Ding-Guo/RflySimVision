...
inline bool AStar::Coord2Index(const Eigen::Vector3d &pt, Eigen::Vector3i &idx) const
{
	...
	if (idx(0) < 0 || idx(0) >= POOL_SIZE_(0) || idx(1) < 0 || idx(1) >= POOL_SIZE_(1) || idx(2) < 0 || idx(2) >= POOL_SIZE_(2))
	{
        ...
        ROS_ERROR("POOL_SIZE_=%d %d %d", POOL_SIZE_(0), POOL_SIZE_(1), POOL_SIZE_(2));
        ROS_ERROR("CENTER_IDX_=%d %d %d", CENTER_IDX_(0), CENTER_IDX_(1), CENTER_IDX_(2));
        ROS_ERROR("center_=%d %d %d", center_(0), center_(1), center_(2));
        ROS_ERROR("inv_step_size_=%d ", inv_step_size_);
