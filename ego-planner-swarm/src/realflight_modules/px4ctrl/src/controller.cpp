...
//      des_acc = des.a + Kv.asDiagonal() * (des.v - V) + Kp.asDiagonal() * (des.p - odom.p);
...
//      double yaw_odom = fromQuaternion2yaw(imu.q);
...
//      u.q = q;
//      u.thrust = 0.7;
      Eigen::Quaterniond imu_odom_q =imu.q * odom.q.inverse();
  ...
  debug_msg_.des_acc_x = des_acc(0);
  debug_msg_.des_acc_y = des_acc(1);
  debug_msg_.des_acc_z = des_acc(2);
  ...
  debug_msg_.q_w = q.w();
    ...
    debug_msg_.odom_q_x = odom.q.x();
    debug_msg_.odom_q_y = odom.q.y();
    debug_msg_.odom_q_z = odom.q.z();
    debug_msg_.odom_q_w = odom.q.w();

    debug_msg_.imu_odom_q_x = imu_odom_q.x();
    debug_msg_.imu_odom_q_y = imu_odom_q.y();
    debug_msg_.imu_odom_q_z = imu_odom_q.z();
    debug_msg_.imu_odom_q_w = imu_odom_q.w();
